...
def MongoIdField(
    ...
    **extra: Any,
    ...
    """
    Mongo specific id field type which includes an ``id_handler``.
    ...
    # stash marker into the extra dict so that the FieldInfo preserves it
    if not id_handler:
        raise ValueError(
            'MongoIdField must be provided an id_handler in order to know how to make new ids when an object is first created, and deserialize id values from mongo outbound')
